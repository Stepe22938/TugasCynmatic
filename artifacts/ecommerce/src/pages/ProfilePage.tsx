...
import { User, Mail, Calendar, ShoppingBag, Edit2, Check, X, LogOut,
         ShieldCheck, Package, Store, Coins } from "lucide-react";
...
export function ProfilePage() {
  ...
  const coins       = Math.floor(totalSpend / 1000);
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-2xl space-y-5">
      ...
      {/* ── Kartu header ────────────────────────────────────────────────── */}
      <div className="bg-gradient-to-br from-primary to-orange-600 rounded-3xl p-6 text-white shadow-lg">
        <div className="flex items-center gap-5">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-orange-200 text-xs mt-1">Bergabung {formatDate(user.createdAt)}</p>
          </div>
        </div>
      </div>
      ...
      {(user.role === "seller" || user.role === "admin") && (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {(user.role === "seller" || user.role === "admin") && (
            <Link href="/seller">
              <div className="flex items-center gap-3 p-4 bg-purple-50 border border-purple-200 rounded-2xl hover:bg-purple-100 transition-colors cursor-pointer">
          ...
          {user.role === "admin" && (
            <Link href="/admin">
              <div className="flex items-center gap-3 p-4 bg-orange-50 border border-orange-200 rounded-2xl hover:bg-orange-100 transition-colors cursor-pointer">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          ...
          { icon: Coins,       label: "Koin Diperoleh",value: `${coins.toLocaleString("id-ID")} 🪙`, color: "text-amber-600" },
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-2xl px-4 py-3 flex items-center gap-3">
        ...
        <div>
          <p className="text-sm font-bold text-amber-800">Koin Saya: {coins.toLocaleString("id-ID")} koin</p>
          <p className="text-xs text-amber-700">Kurs: 1.000 rupiah = 1 koin · Total belanja {formatPrice(totalSpend)}</p>
        </div>
      </div>
