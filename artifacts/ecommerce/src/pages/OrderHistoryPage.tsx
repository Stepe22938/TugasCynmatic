 ...
 * Halaman riwayat pesanan / history pembelian user.
 *
 * Fitur:
 * - Menampilkan daftar semua pesanan yang sudah dibayar
 * - Setiap pesanan menampilkan daftar produk yang dibeli
 * - Tombol "Tulis Ulasan" untuk tiap produk (jika belum pernah di-review)
 * - Tampilkan ulasan yang sudah ditulis jika sudah ada
 * - Modal ReviewForm untuk mengisi/edit ulasan
 */
...
/**
 * Menampilkan bintang rating secara statis (bukan interaktif).
 * Digunakan untuk menampilkan review yang sudah ada.
 */
...
/** Format tanggal ISO ke format yang mudah dibaca (Indonesia) */
...
export function OrderHistoryPage() {
   ...
   * - reviewTarget: pesanan & produk yang sedang di-review
   */
  ...
  /** Buka modal review untuk produk tertentu */
  const openReview = (
    orderId: string,
    product: { id: number; name: string; image: string },
    existingReview?: Review
  ) => {
    setReviewTarget({ orderId, product, existingReview });
  };

  /** Tangani submit review dari modal */
    ...
    return (
      <div className="container mx-auto px-4 py-16 flex flex-col items-center justify-center text-center">
        ...
        <p className="text-muted-foreground mb-8">
          Anda belum pernah melakukan pembelian. Mulai belanja sekarang!
        </p>
        <Link href="/">
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-3xl">
      ...
      {/* Daftar pesanan — diurutkan dari terbaru */}
              ...
              {order.items.map((item) => {
                ...
                return (
                  <div
                    ...
                    data-testid={`item-order-product-${item.id}`}
                  >
                    <div className="flex gap-3 items-start">
                      ...
                      {/* Tombol tulis ulasan atau badge sudah di-review */}
                      <div className="flex-shrink-0">
                        {!review ? (
                          ...
                          <button
                            ...
                            data-testid={`button-edit-review-${item.id}`}
                    ...
                    {/* Tampilkan ringkasan ulasan jika sudah ada */}
                    {review && (
                      <div
                        className="mt-3 p-3 rounded-xl bg-muted/40 border"
                        data-testid={`review-summary-${item.id}`}
                      >
                        {/* Rating & Status */}
                        ...
                        {review.mediaFiles.length > 0 && (
                          <div className="flex gap-2 mt-2">
                            {review.mediaFiles.map((file, i) => (
                              <div
                                key={i}
                                className="w-14 h-14 rounded-lg overflow-hidden border bg-muted"
                                data-testid={`review-media-${item.id}-${i}`}
                              >
                                {file.type === "image" ? (
                                  <img
                                    src={file.preview}
                        ...
                        <p className="text-[10px] text-muted-foreground mt-2">
                          Diulas pada {formatDate(review.createdAt)}
                        </p>
                      </div>
                    )}
                  </div>
