...
export function MyDompetPage() {
  ...
  return (
    <div className="min-h-[calc(100vh-80px)] bg-[#f5f7fa] pb-24">
      ...
      <div className="max-w-3xl mx-auto px-4 -mt-8 relative z-10 space-y-6">
        ...
        {isTopUpOpen && (
          <div className="bg-white rounded-3xl shadow-xl p-6 border border-[#108ee9]/20 animate-in fade-in slide-in-from-top-4">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-black text-gray-800 flex items-center gap-2">
                <Plus className="h-5 w-5 text-[#108ee9]" /> Isi Saldo MyDompet
              </h3>
              <button onClick={() => { setIsTopUpOpen(false); setTopUpStep("amount"); }} className="text-gray-400 hover:text-gray-600"><Plus className="h-5 w-5 rotate-45" /></button>
            </div>
            ...
            {topUpStep === "amount" ? (
              <div className="space-y-4">
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="0"
                    className="w-full pl-12 pr-4 py-4 bg-gray-50 border-2 border-gray-100 rounded-2xl text-2xl font-black focus:outline-none focus:border-[#108ee9] transition"
                  />
                </div>
                <div className="grid grid-cols-3 gap-2">
                  {[50000, 100000, 200000, 500000, 1000000, 2000000].map(amt => (
                    <button 
                      ...
                      className="py-3 px-1 border-2 border-gray-100 rounded-xl text-xs font-bold hover:bg-blue-50 hover:border-blue-200 transition"
                    >
              ...
              <div className="space-y-4">
                <p className="text-sm font-bold text-gray-600">Pilih Metode Pembayaran:</p>
                <div className="space-y-2">
                  {[
                    ...
                    <button key={m.id} onClick={handleTopUpConfirm} className="w-full flex items-center gap-4 p-4 border-2 border-gray-100 rounded-2xl hover:border-[#108ee9] hover:bg-blue-50 transition group">
                      <div className="w-10 h-10 bg-gray-100 rounded-xl flex items-center justify-center group-hover:bg-white">{m.icon}</div>
        ...
        {isSendOpen && (
          <div className="bg-white rounded-3xl shadow-xl p-6 border border-[#108ee9]/20 animate-in fade-in slide-in-from-top-4">
            <div className="flex justify-between items-center mb-4">
              ...
              <button onClick={() => setIsSendOpen(false)} className="text-gray-400 hover:text-gray-600"><Plus className="h-5 w-5 rotate-45" /></button>
            </div>
            <div className="space-y-4">
              <div className="space-y-1">
                ...
                <div className="flex gap-2 overflow-x-auto pb-2 no-scrollbar">
                  {friends.map(f => (
                    <button key={f.id} onClick={() => setSendData({...sendData, toId: f.id, toName: f.name})}
                      className={`flex-shrink-0 flex flex-col items-center gap-1 p-2 rounded-xl border-2 transition ${sendData.toId === f.id ? "border-[#108ee9] bg-blue-50" : "border-gray-50"}`}>
                      <div className="w-10 h-10 bg-gray-100 rounded-full flex items-center justify-center text-xs font-bold">{f.name.charAt(0)}</div>
                ...
                <input 
                  ...
                  className="w-full px-4 py-3 bg-gray-50 border-2 border-gray-100 rounded-xl text-sm focus:outline-none focus:border-[#108ee9]"
                />
              </div>
              <div className="space-y-1">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="0"
                    className="w-full pl-10 pr-4 py-3 bg-gray-50 border-2 border-gray-100 rounded-xl text-lg font-black focus:outline-none focus:border-[#108ee9]"
                  />
                </div>
              </div>
        ...
        {isRequestOpen && (
          <div className="bg-white rounded-3xl shadow-xl p-6 border border-[#108ee9]/20 animate-in fade-in slide-in-from-top-4">
            <div className="flex justify-between items-center mb-4">
              ...
              <button onClick={() => setIsRequestOpen(false)} className="text-gray-400 hover:text-gray-600"><Plus className="h-5 w-5 rotate-45" /></button>
            </div>
            <div className="space-y-4">
              <input 
            ...
            <div className="divide-y divide-gray-50">
              {transactions.slice(0, 10).map(tx => (
                <div 
                  ...
                  className="p-4 flex items-center gap-4 hover:bg-gray-50/50 transition cursor-pointer active:bg-gray-100"
                >
                  ...
                  <div className={`text-sm font-black ${tx.amount > 0 ? "text-green-600" : "text-gray-900"}`}>
            ...
            <div className="bg-white w-full max-w-sm rounded-[40px] overflow-hidden shadow-2xl animate-in zoom-in-95 duration-200">
              <div className="bg-[#108ee9] p-8 text-white text-center relative">
                <button 
                  ...
                  className="absolute right-6 top-6 w-8 h-8 flex items-center justify-center bg-white/20 rounded-full hover:bg-white/30 transition"
                >
                  <Plus className="h-5 w-5 rotate-45" />
                </button>
                <div className="w-16 h-16 bg-white/20 rounded-3xl flex items-center justify-center mx-auto mb-4 border border-white/20">
              ...
              <div className="p-8 space-y-6">
                <div className="space-y-4">
                  <div className="flex justify-between items-start">
                    <span className="text-[10px] font-black text-gray-400 uppercase">Deskripsi</span>
                  ...
                  <div className="flex justify-between items-center">
                    <span className="text-[10px] font-black text-gray-400 uppercase">Tanggal</span>
                    <span className="text-sm font-bold text-gray-700">
                  ...
                  {selectedTx.senderName && (
                    <div className="flex justify-between items-center">
                      <span className="text-[10px] font-black text-gray-400 uppercase">Pengirim</span>
                      <span className="text-sm font-bold text-blue-600">{selectedTx.senderName}</span>
                    </div>
                  )}
                  {selectedTx.recipientName && (
                    <div className="flex justify-between items-center">
                      <span className="text-[10px] font-black text-gray-400 uppercase">Penerima</span>
                      <span className="text-sm font-bold text-[#108ee9]">{selectedTx.recipientName}</span>
                    </div>
                  )}
                  
                  <div className="flex justify-between items-center">
                    <span className="text-[10px] font-black text-gray-400 uppercase">ID Transaksi</span>
