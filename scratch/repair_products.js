...
const staticProducts = [
  {
    ...
    longDescription:
      "Sepatu sneakers premium yang dirancang untuk kenyamanan maksimal sepanjang hari. Menggunakan bahan mesh breathable yang membuat kaki tetap sejuk, sol berbasis EVA yang ringan namun tahan lama, serta desain modern yang cocok untuk segala kesempatan — dari aktivitas santai hingga olahraga ringan.",
    price: 299000,
  ...
  },
  {
    ...
    longDescription:
      "Tas ransel canvas premium dengan kapasitas besar yang ideal untuk mahasiswa dan pekerja kantoran. Dilengkapi kantong laptop berlapisan busa hingga 15 inci, beberapa kompartemen terorganisir, serta bahan canvas tebal yang tahan air dan goresan.",
    price: 189000,
  ...
  },
  {
    ...
    longDescription:
      "Kemeja flanel dengan motif kotak klasik yang tak lekang oleh waktu. Dibuat dari bahan flanel 100% cotton brushed yang terasa lembut di kulit dan memberikan kehangatan ekstra di cuaca dingin. Cocok dipakai sebagai outer maupun kemeja.",
    price: 159000,
  ...
  },
  {
    ...
    longDescription:
      "Jam tangan bergaya minimalis dengan dial bersih tanpa angka yang memberikan kesan elegan modern. Menggunakan mesin Quartz Jepang yang akurat dan tahan lama, tali berbahan kulit sintetis premium yang nyaman di pergelangan tangan, serta kaca mineral anti-goresan. Tahan percikan air hingga 3 ATM.",
    price: 459000,
...
async function run() {
  ...
  try {
    ...
    console.log("Connected to MariaDB VPS!");

    for (const p of staticProducts) {
      console.log(`Updating static product ID ${p.id}...`);
      await conn.query(
        `UPDATE products 
         SET description = ?, 
             longDescription = ?, 
    ...
    console.log("Updating test product ID 10000...");
    await conn.query(
      `UPDATE products 
       SET description = ?, 
           longDescription = ?, 
       ...
       WHERE id = 10000`,
      [
        "Ini adalah sepatu tes untuk uji coba sistem Cynmatic.",
        "Detail lengkap sepatu tes untuk memastikan visualisasi produk dan flow transaksi berjalan normal.",
        "https://images.unsplash.com/photo-1542291026-7eec264c27ff?w=600&h=500&fit=crop", // use beautiful red sneaker as placeholder
        JSON.stringify(["https://images.unsplash.com/photo-1542291026-7eec264c27ff?w=600&h=500&fit=crop"]),
        JSON.stringify([{ label: "Bahan", value: "Canvas + Mesh" }, { label: "Kualitas", value: "High Grade" }]),
        "Admin Toko",
        15 // give it some stock so it doesn't just say habis
      ]
    );

    console.log("Products repaired successfully!");
    await conn.end();
  } catch (err) {
    console.error("Failed to repair products:", err);
  }
}
